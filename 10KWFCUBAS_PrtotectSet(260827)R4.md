---
name: 10KWFCUBAS_PrtotectSet(260827)R4
title: 배터리 보호 설정값 — 10kWFC NCM 22S1P Pack BMS
revision: 260827 R4  (직전: 260827 R3)
source_xlsx: F:/2026_Project/01 현대자동차_10kWFC_2차년도/00 설계검토/04 보호설정/10KWFCUBAS_PrtotectSet(260827)R4.xlsx
...
| 구분 | 항목 | R3 | R4 | 비고 |
|:--|:--|:--|:--|:--|
| 값 변경 | `Bsa_FltUv` (팩 저전압 폴트) | 62.7 V | **58.3 V** | 셀 폴트 2.65 V × 22S 연동 |
| 값 변경 | `Bsa_FltCellUv` (셀 저전압 폴트) | 2.85 V | **2.65 V** | 셀 기준 보호 최대치 변경 |
| 항목 삭제 | `Bsa_FltCharCellUt` (저온 충전 금지, 15 ℃) | No.29 | **삭제** | 규약 비트 미할당 상태로 유지되던 항목 정리 |
| 상태 변경 | `Bsa_FltCellIR_OV` · `Bsa_FltIMD_Err` | 확인 요청 | **2nd 반영 예정** | 2차년도 범위에서 제외 |

**항목 수 38 → 37** (Warning 15 / Fault 22).  
`Bsa_FltCharCellUt` 삭제로 **폴트 항목 번호(No.16~34)가 0x603 비트 번호와 완전히 일치**하게 되었습니다 (오프셋 0).
...
| 29 | `Bsa_FltCellUnbalTmp` | 10 | [ºC] | 이상 | 즉시 = 0 | bit 29 | `C_CellTempDIV_Fault / _FaultDelay` | **20.0** / **5000** | **값·Delay 불일치** |
...
|:--|:--|:--|:--|
| `Bsa_FltUv` | **58.3 V** | 62.7 | **R4 신규** — `C_PackVoltUN_Fault` 58.3 으로 수정 |
| `Bsa_FltCellUv` | **2.65 V** | 2.85 | **R4 신규** — `C_CellVoltUN_Fault` 2.65 로 수정 |
| `Bsa_FltCellUnbalTmp` | 10 ℃ | **20.0** | 260810 펌웨어가 선행 변경. 표/펌웨어 중 기준 확정 필요 |
...
/* [3] 기준 확정 후 정렬 */
#define C_CellTempDIV_Fault        20.0   /* 표는 10.0 — 확정 필요 */
...
{
 "revision": "260827R4",
 "source": "10KWFCUBAS_PrtotectSet(260827)R4.xlsx",
