# 10kWFCU BAP2nd Protocol — R15 (2026-09-10)

> **문서 버전:** R15  
> **양식:** 10kWFCU_BAP2ndProtocol R32 준용 · **물리계층:** CAN 2.0A Standard(11-bit ID) · LSB=intel · DLC 8byte  
> **기준 펌웨어:** 47 HDFC 10kW NCM Pack BMS (F28069) — main.c 260910 반영 (VER 0.22)  
> **검증:** 124 시그널 / 12 메시지, StartBit·Length·겹침 오류 **0건** · 0x608 은 64bit 전부 사용(미사용 비트 없음)
...
| 구분 | ID | 항목 | R14 | R15 |
|---|---|---|---|---|
| 신규 | 0x608 | `BMS_NvrSRStatus` | byte4 예약(0) | **bit32~39 (byte4)** — NVRAM 상태 레지스터 원본 |
| 신규 | 0x608 | `BMS_NvrOk` | byte5 예약(0) | **bit40~47 (byte5)** — NVRAM 부팅 자체진단 결과 |

### 변경 사유

R14 에서 0x608 의 **byte4~5 를 예약(0)** 으로 남겨 두었다. 260910 BATIC 통신 장애 추적 과정에서
**NVRAM 무응답**(`BMS_SpiTimeout` 이 10 으로 관측)이 드러났고, 이는 부팅 중 `NVR_Init()` /
`NVRAM_SelfTest()` 가 SPI 대기 루프에서 멈춰 `main()` 의 `while(1)` 진입 자체를 막은 원인이었다.

NVRAM 이상은 **SOC 저장 실패**로 이어지므로 디버거 없이 상시 관측할 수단이 필요하다.
펌웨어에 이미 부팅 진단값 두 개(`NVRAllRegs.SRStatus` / `NVRAllRegs.NvrOk`)가 정의되어 있어,
이를 예약 자리에 그대로 배치한다.

- `BMS_NvrSRStatus` : NVRAM 상태 레지스터 원본. **0xFF = 칩 무응답**, 0x00 = MISO Low 고착,
  bit3:2 = BP(쓰기보호) 비트. `NVR_UnLock()` 수행 후 값이 기록된다.
- `BMS_NvrOk` : 부팅 1회 write→read 왕복 진단. **1 = 일치(정상)**, 0 = 실패.

`BMS_SpiTimeout`(byte3) 과 함께 보면 **"SPI 계층 문제인지 / NVRAM 칩 무응답인지"** 가 갈린다.

> ⚠️ 두 값은 `NVR_Init()` / `NVRAM_SelfTest()` 가 부팅 시 호출될 때만 갱신된다.
> 해당 호출이 비활성인 빌드에서는 초기값 0 이 송신되므로 판정에 쓰지 않는다.

### 0x608 배치 변경 (R14 → R15)

| bit | byte | R14 | R15 |
|---|---|---|---|
| 32~39 | 4 | 예약(0) | **`BMS_NvrSRStatus`** |
| 40~47 | 5 | 예약(0) | **`BMS_NvrOk`** |

> 나머지 바이트(0~3, 6~7)는 R14 와 동일하다. **예약 자리를 채우는 변경이므로
> R14 파서는 그대로 동작**하며, FCU DBC 는 신호 2개 추가만 하면 된다.
>
> 📎 전송 규칙·시그널 배치·Value Table·판독 예시는 아래 **「0x608 최종 배치 (R15 기준, 펌웨어 정합)」** 참조.
...
| 32~39 | **4** | 8 | **`BMS_NvrSRStatus`** | `CANTxC` 하위 | `NVRAllRegs.SRStatus & 0x00FF` — NVRAM 상태 레지스터 **[R15 신규]** | 0~255 | 0xFF=무응답 |
| 40~47 | **5** | 8 | **`BMS_NvrOk`** | `CANTxC` 상위 | `NVRAllRegs.NvrOk & 0x00FF` — 부팅 왕복 진단 **[R15 신규]** | 0~1 | **1** |
...
CANARegs.CANTxC = ComBine((NVRAllRegs.NvrOk & 0x00FF),(NVRAllRegs.SRStatus & 0x00FF));  /* byte4 SRStatus, byte5 NvrOk */
...
#### (4) Value Table — NVRAM 진단 (R15 신규)

**`BMS_NvrSRStatus`** (byte4) — MR25H40 상태 레지스터 원본
`bit7=WPEN`, `bit3:2=BP1:BP0`, `bit1=WEL`

| 값 | 약어 | 의미 | 판정 |
|---|---|---|---|
| `0x00` | **MISOLO** | MISO Low 고착 | 배선 이상 (MISO/GPIO55) |
| `0x02` | **NORM** | BP=00, 쓰기보호 해제됨 | **정상** |
| `0x04` | **BP1Q** | BP=01, 상위 1/4 보호 잔존 | 쓰기보호 해제 실패 |
| `0x08` | **BP2Q** | BP=10, 상위 1/2 보호 잔존 | 쓰기보호 해제 실패 |
| `0x0C` | **BPALL** | BP=11, 전 영역 보호 잔존 | 쓰기보호 해제 실패 |
| `0x80` | **WPEN** | 하드웨어 쓰기보호 활성 | `NVR_Lock()` 오호출 의심 |
| `0xFF` | **NORSP** | 칩 무응답 | 미실장·CS(GPIO11)·전원 |

> `BP` 잔존(`0x04`/`0x08`/`0x0C`)은 **칩은 응답하지만 쓰기가 무시되어** SOC 저장만 조용히
> 실패하는 상태다. 펌웨어의 `(sr & 0x0C) != 0` 판정과 대응한다.
>
> ⚠️ `WEL`(bit1)은 쓰기 완료 후 자동 클리어되는 칩이 있어 정상값이 `0x00` 으로 관측될 수 있다.
> 그 경우 `0x00` 을 `NORM` 으로 재정의하고, MISO 고착은 `BMS_NvrOk` 와 조합해 판별한다.

**`BMS_NvrOk`** (byte5) — 부팅 1회 write→read 왕복 진단

| 값 | 약어 | 의미 |
|---|---|---|
| `0x0` | **NOTRUN** | 미실행 — 자체진단이 호출되지 않음 (초기값) |
| `0x1` | **OK** | 왕복 일치 (정상) |
| `0x2` | **NG** | 왕복 불일치 — NVRAM 이상 |

> 초기값(0)과 실패값(2)을 분리해, `NVR_Init()`/`NVRAM_SelfTest()` 가 비활성인 빌드의
> **미실행 상태를 고장으로 오독하지 않도록** 했다. (`NVRAM.c` 실패값 0→2 변경)

**두 값 조합 판정**

| `BMS_NvrOk` | `BMS_NvrSRStatus` | 판정 |
|---|---|---|
| NOTRUN(0) | 0x00 | 자체진단 미실행 — **판정 보류** |
| OK(1) | 0x02 (NORM) | **NVRAM 정상** |
| NG(2) | 0xFF (NORSP) | 칩 무응답 — 미실장·CS·전원 |
| NG(2) | 0x04/0x08/0x0C (BP*) | 응답하나 쓰기보호 잔존 — 쓰기 무시 |
| NG(2) | 0x00 (MISOLO) | MISO 배선 이상 |

#### (5) 판독 예시

수신 페이로드가 `7B 64 32 00 FF 00 00 00` 인 경우:
...
| 4 | FF | 255 | `BMS_NvrSRStatus` = 0xFF → **NVRAM 무응답** |
| 5 | 00 | 0 | `BMS_NvrOk` = 0 → **자체진단 실패** |
...
| byte4 = 0xFF | NVRAM 칩 무응답 | NVRAM 전원·CS(GPIO11)·배선 점검. SOC 저장 불가 상태 |
| byte4 = 0x00 | MISO Low 고착 | SPI MISO(GPIO55) 배선 점검 |
| byte4 bit3:2 ≠ 0 | 쓰기보호(BP) 걸림 | `NVR_UnLock()` 동작 확인 |
| byte5 = 0 | 부팅 왕복 진단 실패 | byte3·byte4 와 함께 원인 판별 |
...
| BSA | BSA9 | 0x608 | 8 | 100 | BMS_NvrSRStatus | 8 | 32 | LSB | Unsigned | 0 | 1 | 0 | 0 | 255 | - | 0x00=MISOLO;<br>0x02=NORM;<br>0x04=BP1Q;<br>0x08=BP2Q;<br>0x0C=BPALL;<br>0x80=WPEN;<br>0xFF=NORSP; | byte4 — CANTxC 하위바이트 = NVRAllRegs.SRStatus & 0x00FF (NVRAM.h). NVRAM 상태 레지스터 원본, 0xFF=칩 무응답 / 0x00=MISO Low / bit3:2=BP(쓰기보호). NVR_Init() 호출 시에만 갱신 [R15 신규] |  | R | T |  |
| BSA | BSA9 | 0x608 | 8 | 100 | BMS_NvrOk | 8 | 40 | LSB | Unsigned | 0 | 1 | 0 | 0 | 2 | - | 0x0=NOTRUN;<br>0x1=OK;<br>0x2=NG; | byte5 — CANTxC 상위바이트 = NVRAllRegs.NvrOk & 0x00FF. 부팅 1회 write→read 왕복 진단. 0=미실행(초기값) / 1=일치 / 2=불일치. NVRAM_SelfTest() 호출 시에만 갱신 [R15 신규] |  | R | T |  |
